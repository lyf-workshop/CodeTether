...
- Agent execution or product-data forwarding through Relay.
...
**Phase 7A — Internet Relay Architecture Foundation** is the current approved implementation phase; it is not yet accepted, frozen, or declared ready. It adds a standalone public Relay with durable cryptographic identity, one-time bounded enrollment, challenge-authenticated reconnect, deterministic connection epochs, bounded heartbeat/presence, reciprocal authorization for rendezvous, revocation, version negotiation, rate/resource limits, safe diagnostics/observability, and production deployment foundations. Controller and Node connections are outbound-only and authenticate cryptographic identity rather than hostname, IP, NAT address, or Alibaba instance identity. Host migration 014 (`relay_controller_configuration`) retains only the bounded private endpoint, pinned Relay identity, transport mode, safe display label, enablement/enrollment state, and lifecycle timestamps for an already-trusted Machine; it never retains the enrollment token, private key, live epoch, presence history, or Agent/product data. A revoked peer can return only through explicit re-enrollment with a newly issued role-scoped one-time token; that operation does not alter the existing Controller ↔ Node pairing.

Phase 7A is deliberately not Internet execution. Relay-connected and Node-present are infrastructure facts only; they do not mean Provider health, Project availability, Machine trust, or execution readiness. No Agent or product payload can be represented by the Relay protocol. The existing direct transport remains authoritative for Codex and restricted Claude execution and retains all frozen Project Location, ownership, no-replay, crash-containment, and diagnostic semantics. The narrow UI may configure and enroll Relay, show safe Relay/presence state, and state explicitly that Internet execution through Relay is not enabled.

Production validation targets the Owner-prepared Alibaba Cloud server with public TLS on TCP 443 and outbound Controller/Node clients. Access details, approved SSH authentication, privilege for the production service lifecycle, domain/certificate state, security-group authorization, and whether the service remains deployed must be Owner-provided; implementation and local/staging evidence must not fabricate that Internet deployment when access is unavailable.

Start with Windows, tray Attention badges/counts, recent-item tray menus, close-behavior settings, notification delivery after explicit Quit, notification history, remote/mobile/browser push, email/chat delivery, custom sounds, schedules, updater/signing, automatic Host restart, drag-and-drop, Project discovery, Project/Location relocation, synchronization, Relay execution or payload forwarding, NAT traversal, richer remote execution/control, LAN discovery shipment, OpenCode/third Providers, cross-Provider handoff, Claude capability parity, and all Phase 7B+ behavior remain unimplemented.
