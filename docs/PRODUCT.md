...
The Host detects local Provider availability once per lifecycle and presents actionable states—available, not installed, unsupported version, misconfigured, or temporarily unavailable—without exposing executable paths or CLI diagnostics. Accepted Phase 6C.1 lets the authenticated remote Node produce the same presentation vocabulary through one bounded version-only discovery operation. Remote snapshots are refreshed on authenticated connection or explicit request and are clearly current, last known, or not yet observed. Detecting a remote CLI does not itself enable execution. Phase 6C.2 separately gates a fixed remote Codex text-streaming/native-resume profile by current authenticated Machine, exact registered Location, tested CLI version, and Node execution support; every other remote capability remains false. New Conversation chooses one executable Agent and stores that Machine and Agent identity durably. Existing Conversations never offer Machine or Provider switching, handoff, delegation, or history replay into another Agent.
