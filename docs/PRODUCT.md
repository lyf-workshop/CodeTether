...
A supported coding-agent runtime/provider. Codex remains the established Provider; the current Phase 5A scope adds a deliberately restricted Claude Code foundation through the same product model. OpenCode remains deferred.
...
Agents expose detected Providers and explicit capabilities through a unified experience. Provider-specific protocol details must not leak into product UI. Codex advertises the established control surface. The Phase 5A Claude Code foundation truthfully advertises only its supported restricted read/search Tool and native-session streaming/resume path; unsupported Approval, interrupt, edit, shell, Diff, model-selection, and Codex-style reasoning controls stay absent. OpenCode remains deferred.
...
The Claude Code Phase 5A execution profile is intentionally safe and minimal. Canonical User input travels to the CLI through stdin rather than a shell command. Only Read, Glob, and Grep are admitted in its noninteractive `dontAsk` profile; text streaming and high-confidence Read/Search Tool activity are normalized into the existing Timeline, while unrecognized Tools remain generic. Restricted execution reuses only an allowlisted, private projection of the user's Claude authentication/endpoint/model configuration and never loads hooks, plugins, MCP, Project settings, or arbitrary environment values. CodeTether does not silently grant write/shell access or manufacture an Approval flow that the CLI integration cannot represent reliably.
...
V1 remains **Codex-first**, while Phase 5A validates that the same product entities and canonical control boundary can host one deliberately restricted second Provider.

Explicitly deferred:

- Claude Code capability parity beyond the Phase 5A foundation, including machine-readable Approval, edit/shell permissions, Diff, interrupt, model selection, and reasoning controls.
...
**Phase 5A — Second Provider Foundation: Claude Code** is the current approved implementation phase; it is not yet accepted, frozen, or declared ready. Protocol v1 now models durable `codex | claude-code` Provider identity, presentation-safe detection/capabilities, provider-neutral Tool kinds, and bounded canonical Provider errors. Migration 007 safely widens existing Codex-only persistence. The Host owns one Provider registry and routes the existing create/Turn/interrupt/read/organization/Search surfaces to each Conversation's immutable Provider while preserving one shared hydration budget. The Claude Code adapter safely detects the CLI, uses its native session create/resume identity, sends Prompts through JSONL stdin with structured process arguments, and normalizes text and high-confidence Tools into the existing event model.

Phase 5A intentionally does not claim Claude Code capability parity. Its safe restricted profile advertises no Approval, interrupt, edit, shell, Diff, model-selection, or Codex-style reasoning control, and the UI must follow those capability facts. Phase 5A acceptance still requires the specified real Claude Code turn/resume, mixed-Provider, background, process-ownership, installed-Desktop, and Codex-regression evidence; implementation alone is not READY.
