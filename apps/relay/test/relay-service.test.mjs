...
test('pinned TLS, enrollment, reconnect, authorization and presence stay separate from Machine trust', async () => {
  await withService(async ({ store, service, tls }) => {
    ...
    assert.equal(metrics.queuedInboundFrames, 0)
    assert.equal(metrics.queuedInboundBytes, 0)
    assert.equal(metrics.pendingOutboundFrames, 0)
    assert.equal(metrics.pendingOutboundBytes, 0)
    assert.equal(metrics.framedQueueFrames, 0)
    assert.equal(metrics.framedQueueBytes, 0)
    assert.equal(metrics.framedQueueFramesHighWaterMark >= 1, true)
    assert.equal(metrics.framedQueueBytesHighWaterMark > 0, true)
    assert.equal(metrics.pendingChannelDataBytes, 0)
    assert.equal('peers' in metrics, false)
    assert.equal('peerIds' in metrics, false)
    assert.equal('fingerprints' in metrics, false)
    assert.equal('channelIds' in metrics, false)
