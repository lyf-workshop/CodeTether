...
Phase 1 Frontend Experience is accepted and frozen as **CodeTether V2 Frontend Core v1**. Phase 2A and Phase 2A.1 are accepted and frozen as **Phase 2A Codex Runtime v1**. **Phase 2B — Client-to-Host Protocol & Local API** now validates a local-only HTTP/SSE boundary, CodeTether-owned public identities, bounded in-memory reconnect behavior, and a non-React client without changing the frozen UI. It adds no persistence, Tauri functionality, remote access, authentication, or non-Codex provider integration. See [`ROADMAP.md`](ROADMAP.md) for phase gates.
