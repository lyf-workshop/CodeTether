...
**Phase 4G.1 — System Tray & Background Runtime Foundation** is implemented; Owner acceptance is pending. Phase 4F.2 is accepted and frozen at `545db8c`, Phase 4F.1 is accepted and frozen at `38481ad`, Phase 4E.2 is accepted and frozen at `84de855`, Phase 4E.1 is accepted and frozen at `afac51a`, and Phase 4D, Phase 4C, Phase 4B, Phase 4A, Phase 3E.1, and the **CodeTether Local Workspace Alpha** remain frozen. The accepted frontend is still **CodeTether V2 Frontend Core v1**, the accepted local runtime is still **Phase 2A Codex Runtime v1**, Protocol v1 remains the **Phase 2B Client ↔ Host Protocol** boundary, and the durable Project/Conversation/Attention data layer remains authoritative. Phase 4G.1 separates main-window visibility from the Desktop process and its owned Host Runtime without moving any business truth into Tauri. The current product boundary includes:
...
- Tray Quit is the only Phase 4G.1 product exit. An idempotent quit guard invokes the existing bounded graceful Host shutdown, removes the tray after owned-child drain, and exits the Desktop process. Close-to-tray does not interrupt a running Turn or expire a process-live Approval; explicit Quit and abnormal process loss retain the established restart/interruption/Approval-expiry semantics.
...
Phase 4C notification delivery remains bounded to the lifetime of the Desktop process and its owned Host, which Phase 4G.1 can now keep running while the main window is hidden. Phase 4F.2 is accepted and frozen. Phase 4G.1 is implemented but must not be described as accepted or frozen until Owner review. It authorizes only the exact close-to-tray lifecycle, minimal Open/Quit tray, centralized existing-window restoration, one-time background education, and compact Desktop-only Settings explanation above. Start with Windows, tray badges or dynamic Attention counts, recent-item menus, a close-behavior setting, notifications after explicit Quit, notification history, push infrastructure, custom window chrome, updating/signing infrastructure, Activity, Machine management, remote operation, and another provider remain unauthorized. Windows session-ending signals enter Tauri's unpreventable exit path and receive a best-effort synchronous Host drain, but the current Tao event loop does not expose `WM_QUERYENDSESSION`; a full bounded graceful guarantee during forced logoff/shutdown remains a later Windows lifecycle validation item.

## Out of Scope

During and after Phase 4G.1, do not implement without a separately approved phase:
