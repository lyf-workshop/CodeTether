...
Phase 8D applies a trusted-Node configuration policy to that unchanged transport and execution-ownership boundary. A Node runs the exact selected Codex installation as its normal non-root operating-system user and leaves Codex in authority over that user's native backend, authentication, session store, and configured MCP servers. CodeTether does not parse, copy, rewrite, enumerate, or expose that configuration. Native MCP startup lifecycle notifications remain private and are ignored rather than treated as a remote protocol violation; they do not add a Machine-protocol Tool surface or change the public remote Codex capability descriptor. Machine TLS, Controller trust, exact installation binding, action idempotency, process ownership, and no-replay behavior remain unchanged.
...
The Phase 8D native historical transcript projection is an additive read path for an already adopted native Conversation. Protocol v1 exposes only normalized, read-only entries under the public Conversation identity; provider-native identity, storage path, boundary, and parser cursor stay private. Host delegates local parsing to the exact Provider metadata adapter and remote parsing to one narrow Machine TLS operation bound to Conversation, Machine, ProjectLocation, Provider, and ProviderInstallation. Relay continues to forward opaque nested TLS records and has no transcript schema or persistence. Repeated identical page reads coalesce with independent caller cancellation, while random short-lived Host cursors prevent Provider cursor or filesystem-offset exposure.
...
Phase 8D adds one exceptional local management lifecycle for the case where all usable Controller credentials have been lost but Node state remains healthy. It is not a Machine, Relay, Host, Web, or remote RPC operation. The installed Node service must first be stopped, which releases active Provider ownership and the exclusive Node-state lock. The same local OS user can then enumerate only safe Controller metadata and explicitly target one trust record; exact interactive confirmation binds the action to the durable Machine identity and public fingerprint suffix. One atomic trust-file replacement removes only that Controller and appends a bounded presentation-safe recovery audit record. Machine/Node identity, Relay enrollment, Provider installations/selections, Projects, and native sessions remain untouched. Recovery only makes the Node pairing-ready; a new Controller still requires the unchanged one-time cryptographic pairing and confirmation flow.
...
Phase 8D migration 018 (`native_transcript_boundary`) adds one nullable, bounded, Provider-private boundary to adopted Conversations only. New adoption captures a content-free Provider-specific split so later Provider-native continuation appended to the same store is not projected twice beside authoritative CodeTether Turns. Existing adopted Conversations retain all bindings and Turns without re-adoption; adapters derive only conservative legacy history and return `partial` when a reliable split is unavailable. No transcript content, file path, Provider cursor, fake Turn, or action identity is persisted.
...
Phase 8D adds a separate first-pairing bootstrap for an already-enrolled Node whose normal listener is not remotely reachable. The Node registers one short-lived, one-time, high-entropy Relay rendezvous while its existing pairing mode is live. A pre-trust Controller proves possession of its newly generated per-Machine key and may open only the exact `pairing_opaque_v1` rendezvous; it cannot observe presence, open `machine_tls_v1`, enumerate peers, or issue a Machine operation. The Relay stores only a capability digest and ephemeral bounded routing state and never receives the six-digit code. Both Direct and Relay adapters establish fresh inner Machine TLS and run the same OPAQUE pairing, transcript, candidate, explicit confirmation, and trust persistence. The unchanged transcript includes the inner TLS exporter, so the Relay cannot splice or replay one pairing stream into another. Only after the Node has durably created Controller trust may it reconcile that exact consumed Controller public key into the ordinary reciprocal Relay grant. Full lifecycle, visibility, rollout, and failure boundaries are recorded in `docs/PHASE8D-RELAY-ASSISTED-FIRST-PAIRING.md`.
