...
**Phase 4G.2 — Windows Session & Background Reliability** is implemented and its bounded evidence collection is complete; Owner acceptance is pending. Phase 4G.1 is accepted and frozen at `cee3a71`, Phase 4F.2 is accepted and frozen at `545db8c`, Phase 4F.1 is accepted and frozen at `38481ad`, Phase 4E.2 is accepted and frozen at `84de855`, Phase 4E.1 is accepted and frozen at `afac51a`, and Phase 4D, Phase 4C, Phase 4B, Phase 4A, Phase 3E.1, and the **CodeTether Local Workspace Alpha** remain frozen. The accepted frontend is still **CodeTether V2 Frontend Core v1**, the accepted local runtime is still **Phase 2A Codex Runtime v1**, Protocol v1 remains the **Phase 2B Client ↔ Host Protocol** boundary, and the durable Project/Conversation/Attention data layer remains authoritative. Phase 4G.1 separates main-window visibility from the Desktop process and its owned Host Runtime without moving any business truth into Tauri; Phase 4G.2 hardens only the approved Windows power/session boundaries around that ownership. The current product boundary includes:
...
- Tray Quit remains the only product exit: an idempotent guard gives the owned Host up to 12 seconds to complete its existing graceful HTTP/SSE, mutation, Runtime, SQLite, Approval, and Codex drain, then terminates only the owned process tree if necessary. X and Alt+F4 remain hide-only and do not interrupt a running Turn or expire a process-live Approval.
...
- Abnormal Desktop/parent loss still relies on `JOB_OBJECT_LIMIT_KILL_ON_JOB_CLOSE` to remove the owned Host/Codex tree, with the established durable restart reconciliation converting incomplete Turns to interrupted history and expiring process-live Approvals on the next launch. There is no automatic Host restart.
...
Phase 4C notification delivery remains bounded to the lifetime of the Desktop process and its owned Host, which accepted Phase 4G.1 keeps running while the main window is hidden. Phase 4G.1 is accepted and frozen at `cee3a71`; Phase 4G.2 is implemented but must not be described as accepted or frozen until Owner review. Its evidence explicitly distinguishes real raw-release Sleep/Wake, real pending-Approval continuity and real Explorer recovery from simulated Win32 message cycles and from unobserved real logoff/shutdown, provider-network-change, active-Turn-at-suspend, dedicated Win+L, and installed-runtime-across-Sleep cases. Phase 4G.2 authorizes only the exact Windows sleep/resume, lock/unlock observation, session-end, crash-reconciliation, and Explorer tray-recovery boundaries above. Start with Windows, tray badges or dynamic Attention counts, recent-item menus, a close-behavior setting, notifications after explicit Quit, notification history, push infrastructure, custom window chrome, updating/signing infrastructure, automatic Host restart, Activity, Machine management, remote operation, and another provider remain unauthorized. Confirmed Windows session end is intentionally a 2-second best-effort termination budget, not the ordinary 12-second graceful Tray Quit guarantee.

## Out of Scope

During and after Phase 4G.2, do not implement without a separately approved phase:
...
- Activity, Machines, Agents, a broader Settings redesign, an advanced New Conversation flow, or any other new product-page content or flow beyond the exact notification preferences and compact Desktop background-running explanation.
