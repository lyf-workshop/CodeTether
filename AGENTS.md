...
**Phase 4F.2 — Search UI & History Discovery** is implemented; Owner acceptance is pending. Phase 4F.1 is accepted and frozen at `38481ad`, Phase 4E.2 is accepted and frozen at `84de855`, Phase 4E.1 is accepted and frozen at `afac51a`, and Phase 4D, Phase 4C, Phase 4B, Phase 4A, Phase 3E.1, and the **CodeTether Local Workspace Alpha** remain frozen. The accepted frontend is still **CodeTether V2 Frontend Core v1**, the accepted local runtime is still **Phase 2A Codex Runtime v1**, Protocol v1 remains the **Phase 2B Client ↔ Host Protocol** boundary, and the durable Project/Conversation/Attention data layer remains authoritative. Phase 4F.2 connects the accepted Project-scoped durable title/User-input Search boundary to the shared Browser/Desktop Conversation List and Rail without changing provider/runtime or native capabilities. The current product boundary includes:
...
Phase 4C remains bounded to best-effort native delivery while the Desktop process and its owned Host are running. Phase 4D, Phase 4E.1, Phase 4E.2, and Phase 4F.1 are accepted and frozen. Phase 4F.2 is implemented but must not be described as accepted or frozen until Owner review. It authorizes only the exact Project-scoped durable Search UI, URL/query state, bounded cursor paging, match explanation, existing public Turn navigation, and organization/realtime integration above. Agent/Tool/Terminal/Diff search, semantic or cross-Project/global Search, old-Turn history pagination, Search history/analytics, Delete, bulk organization, tags/folders/groups, tray behavior, closed-app notifications, a Notification Center/history, push infrastructure, custom window chrome, updating/signing infrastructure, Activity, Machine management, remote operation, and another provider remain unauthorized.

## Out of Scope

During and after Phase 4F.2, do not implement without a separately approved phase:
