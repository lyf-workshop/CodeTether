...
Phase 1 Frontend Experience is accepted and frozen as **CodeTether V2 Frontend Core v1**, including the Design System, AppShell, Desktop Inbox v1, Desktop Conversations v1, and Conversation Workspace v1. **Phase 2A.1 — Codex Runtime Semantics & Approval Validation** is active after the accepted Runtime Spike. It validates local approval, continuity, interruption, failure, cleanup, identity, and bounded delivery semantics without connecting real runtime data to React. It adds no product page, browser transport, persistence, Tauri functionality, remote access, or non-Codex provider integration. See [`ROADMAP.md`](ROADMAP.md) for phase gates.
