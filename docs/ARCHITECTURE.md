...
**Phase 4G.1 — System Tray & Background Runtime Foundation** is implemented; Owner acceptance is pending. The Rust shell creates one native tray after Host readiness, treats main-window X/Alt+F4 as hide rather than application exit, restores the same ready window through one function for Tray/notification/single-instance activation, and reserves the existing bounded graceful Host shutdown for an idempotent explicit Tray Quit. This changes window/application lifecycle only; Host, Runtime, Attention, notification, and provider truth remain where they were.

No Project discovery/scanning or rename/relocate, cross-Project/global Search, Agent/Tool/Terminal/Diff/Approval or semantic Search, Search history/analytics, Conversation Delete/bulk organization/tags/folders/groups/old-Turn history pagination, read/unread Inbox history, remote access, authentication, multiple-Machine Project model, notifications after explicit application exit, updater, additional provider, or general filesystem bridge exists. SQLite remains local Host data, not a remote or multi-user service.
...
apps/desktop               Tauri v2 window/tray, packaging, owned Host supervision, exact directory picker, and bounded notification delivery
...
Phase 4G.1 makes window visibility independent of application and Runtime liveness. After Host readiness, Rust creates exactly one built-in Tauri tray from the configured CodeTether icon. Its surface is intentionally bounded to a `CodeTether` tooltip, left-click restore, `打开 CodeTether`, and `退出 CodeTether`; it has no Attention projection, badge, recent-item model, provider control, or Web-owned creation lifecycle.
...
`packages/protocol` defines Protocol v1 identifiers, commands, records, events, safe errors, capabilities, ordering, reconnect cursors, the strict Project-scoped `ConversationSummary`, bounded durable Conversation-detail response, Conversation organization mutations, durable Conversation Search query/result/cursor contracts, and durable Attention contracts with TypeScript and Zod. `packages/client` implements the matching HTTP/SSE consumer—including Conversation organization, Search, and Attention reads/mutations—without React. Provider wire payloads remain behind adapters, and unknown provider notifications are diagnostics rather than public events.
