...
const repositoryDirectory = resolve(relayDirectory, '..', '..')
const outputDirectory = join(relayDirectory, 'dist', 'sea')
const temporaryDirectory = join(repositoryDirectory, '.tmp', 'relay-sea')

export function createRelayBuildId(revision, dirty) {
...
export function relayArtifactName(
  ...
  if (
    ...
    throw new Error('Relay artifact platform or architecture is invalid')
  }
  return `codetether-relay-${platform}-${architecture}${platform === 'win32' ? '.exe' : ''}`
}
...
async function main() {
  ...
  const buildId = createRelayBuildId(revision, dirty)
  const bundlePath = join(temporaryDirectory, 'codetether-relay.mjs')
  const seaConfigPath = join(temporaryDirectory, 'sea-config.json')
  const artifactPath = join(outputDirectory, relayArtifactName())

  await mkdir(temporaryDirectory, { recursive: true })
  await mkdir(outputDirectory, { recursive: true })
  await build({
    entryPoints: [join(relayDirectory, 'src', 'main.ts')],
    outfile: bundlePath,
    bundle: true,
    platform: 'node',
    format: 'esm',
    ...
    define: {
      __CODETETHER_RELAY_VERSION__: JSON.stringify(buildId),
    },
    logLevel: 'info',
    sourcemap: false,
    minify: false,
...
function assertDirectSeaBuildAvailable() {
  ...
  if (major < 25 || (major === 25 && minor < 5)) {
    throw new Error(
      'CodeTether Relay SEA builds require Node 25.5+ for official --build-sea support',
    )
  }
}
