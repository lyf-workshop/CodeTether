...
**Phase 7A — Internet Relay Architecture Foundation** is the current approved implementation scope; it is not yet accepted, frozen, or declared ready. It adds a standalone Internet Relay and outbound-only authenticated Controller/Node control connections for enrollment, liveness, presence, and authorized rendezvous. Direct Machine transport remains the only execution path. Phase 7A cannot carry Prompt, Conversation, Provider, Tool, filesystem, shell, Terminal, or arbitrary payload data and does not authorize Phase 7B, Relay execution, NAT traversal, Mobile, Push, accounts, or a generic tunnel.

No Project discovery/scanning, Project or Location rename/relocate, multi-root Location on one Machine, cross-Project/global Search, Agent/Tool/Terminal/Diff/Approval or semantic Search, Search history/analytics, Conversation Delete/bulk organization/tags/folders/groups/old-Turn history pagination, read/unread Inbox history, remote execution beyond the exact accepted direct Codex/restricted-Claude profiles, LAN discovery, generic remote Provider data, Machine switching, notifications after explicit application exit, updater, OpenCode/third Provider, cross-Provider handoff, Claude capability parity, Relay execution, or general filesystem bridge exists. Host SQLite remains local durable product state, not a remote or multi-user service; the Relay owns a separate minimal infrastructure registry only.
...
apps/relay                 Standalone TLS Relay service, minimal infrastructure persistence, connection ownership, presence, rendezvous, and operations
...
packages/machine-transport Private purpose-specific authenticated direct Host ↔ Node protocol
packages/relay-protocol    Versioned bounded Relay control messages, framing, identity transcripts, and shared client/server primitives
...
`apps/host` owns the loopback HTTP/SSE server, Provider registry/runtimes, live state, Phase 3A SQLite boundary, Phase 3B.1 durable Project registry, Phase 3C.1 durable Conversation index/title rules, Phase 3C.1.1 cold detail/hydration boundary, Phase 3D.1 durable Attention projection, Phase 4E.1 Conversation organization model, Phase 4F.1 durable Search projection/read, Phase 5A Provider selection, Phase 6D canonical failure/health composition, and the Phase 7A Controller Relay client. It allocates public identities, maps them to private Provider identities, validates actions and Project workspaces, reconstructs bounded live or cold views from durable Turn records, applies Provider-independent organization writes, searches durable title/User-input text without runtime admission, hydrates control state on demand, selects only the immutable owning adapter for control, validates structured upstream failures, records bounded health observations, sequences client events, and performs bounded fanout/replay. Relay configuration and connection state stay separate from Machine direct transport and Provider runtime ownership. The same Host entry supports standalone Browser development and the production Desktop sidecar; Desktop-managed mode changes lifecycle wiring and Origin input, not business behavior.

Project discovery/import UX, Location relocation, multi-root Locations on one Machine, generalized durable action logging, full-history pagination, richer remote Claude capability parity, and richer remote Codex capability parity remain planned rather than implemented. Migration 012 is only the narrow Turn-start admission association described above, and migration 013 is only the latest bounded presentation-safe Provider execution-health observation. Remote identity/trust, bounded authenticated endpoint recovery, purpose-specific remote Location validation, Provider description, the fixed Codex text profile, and the restricted Claude profile exist only at the Machine connection boundary.

## Client-to-Host Protocol

`packages/protocol` defines Protocol v1 identifiers, commands, records, events, safe errors with strict `CanonicalFailure`, Provider descriptors/capabilities with optional bounded execution health, canonical Tool kinds, ordering, reconnect cursors, strict Project and Project Location records/mutations, the Project-scoped `ConversationSummary`, bounded durable Conversation-detail response, Conversation organization mutations, durable Conversation Search query/result/cursor contracts, durable Attention contracts, and bounded presentation-safe Relay status/configuration mutations with TypeScript and Zod. `packages/client` implements the matching HTTP/SSE consumer—including Project Location registration, remote Provider refresh, Conversation organization, Search, Attention, ordinary Conversation creation, and Host-owned Relay configuration/enrollment/status—without React or Provider-specific or Relay-socket methods. The purpose-specific direct Host ↔ Node Location-validation, Provider-description, Codex execution, and Claude execution envelopes remain private to `packages/machine-transport`; the separate Phase 7A Internet control union remains in `packages/relay-protocol`. Raw Provider payloads and private native session identities remain behind Node/Host adapters and never become Web or Relay data. Strict mixed-revision compatibility is not silently negotiated: an incompatible Protocol or Relay peer fails closed instead of weakening validation or mutating durable trust.

## Agent Adapter Boundary

Phase 5A introduces a narrow adapter boundary only after Codex and Claude Code provide a second concrete case. `AgentHostRuntime` carries the common Host operations—create/resume session, start Turn, optional interrupt, event/failure subscriptions, process-live Approval callbacks, cold-session disposal, and shutdown. A Host-owned `ProviderRegistry` maps the immutable durable Provider to one runtime and one public descriptor. Session lookup keys combine Provider identity with the private Provider session identity, so equal opaque IDs from different Providers cannot collide. Provider-specific process, transport, parsing, permission, lifecycle, and structured-error classification code remains inside its adapter; Phase 6D adds only the shared controlled failure result, not a Host/Web Provider-prose parser.
...
- The Host API remains a loopback-only service with local SQLite records. Browser development may launch it separately; CodeTether Desktop packages and owns the same Host as a revision-coupled sidecar. The bounded direct Machine client reaches a manually specified CodeTether Node, while the separate Phase 7A Controller client may make only an outbound TLS control connection to a configured Relay. Web does not connect to the Internet Relay directly.
- Codex remains the accepted full-control integration and was verified against local `codex-cli 0.149.1`. Phase 5A's Claude Code foundation is frozen at `a1329f2`, Phase 5B's evidence-driven capability matrix is accepted and frozen at `5dac13c`, remote reliability is accepted and frozen at Phase 6C.4 commit `01e749c`, and Phase 6D diagnostics are accepted and frozen at `cb2c412`. Phase 7A may report Relay diagnostics but cannot broaden Provider capabilities or execution routing.
