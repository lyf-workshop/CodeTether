...
**Phase 5B — Claude Code Capability Expansion** is the current approved implementation scope; it is not yet accepted, frozen, or declared ready. Phase 5A is accepted and frozen at `a1329f2`. Protocol v1 continues to define durable `codex | claude-code` identity, presentation-safe Provider descriptors/capabilities, provider-neutral Tool kinds, and canonical Provider errors. Migration 007 remains unchanged, one Host-owned registry selects the adapter for each immutable Conversation Provider, and both adapters share the existing Runtime admission budget and unified product API. Phase 5B keeps Claude's bounded detection, structured process arguments/JSONL stdin, native session create/resume, and normalized text/Tool events; it adds only Provider-owned effort options and richer presentation-safe Tool clues proven by the installed CLI. Approval, reliable interrupt, edit, shell, Diff, and model selection remain unsupported.
...
- Codex remains the accepted full-control integration and was verified against local `codex-cli 0.149.1`. Phase 5A's Claude Code detection and restricted real execution/resume path are accepted and frozen at `a1329f2`; Phase 5B remains subject to its own real capability, resume, concurrency, background, installed, and regression evidence.
