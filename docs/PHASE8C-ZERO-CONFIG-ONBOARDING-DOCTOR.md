...
Opening Doctor starts no inference and no continuous polling. `Check again` uses existing bounded/coalesced Provider/connectivity refresh plus purpose-specific exact ProjectLocation validation, with fixed Machine and validation fanout. Codex and Claude checks may run concurrently where safe, and one failure does not cancel the other. Phase 8C deliberately adds no separate inference-based backend-test button; current backend truth remains the Phase 8B observation established by its authoritative lifecycle or real explicit execution boundary.
