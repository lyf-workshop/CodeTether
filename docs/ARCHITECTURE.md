...
**Phase 7B — Authenticated Relay Transport** is the current approved implementation scope; it is not yet accepted, frozen, or declared ready. It carries the existing authenticated Machine protocol through one purpose-bound, ephemeral Relay channel when the direct route is unavailable. Direct remains first, an active Machine session or Turn never changes transport, and the Relay adds no Provider capability, generic tunnel, arbitrary destination, account authority, Machine trust, automatic Prompt replay, or Phase 7C security claim.
...
The Relay routes only bounded Machine TLS ciphertext between the exact authorized Controller and Node epochs. The end peers still authenticate the frozen Machine identities and protocol, and only the existing Node dispatcher can admit Project Location, Provider session, or Turn operations.
...
The Relay forwards the existing end-to-end Machine TLS ciphertext, so the Relay application does not decode Prompts, Provider output, Tools, Project paths, credentials, or native Provider sessions. It can still observe enrolled identities/roles, authorized relationships, connection and channel timing, channel counts, frame sizes, and byte volumes, and it can delay, drop, reorder, or terminate traffic. Phase 7B does not claim the broader compromised-Relay, traffic-analysis, forward-secrecy/rekey, or key-compromise guarantees reserved for Phase 7C. The complete Phase 7B model is documented in `docs/PHASE7B-THREAT-MODEL.md`.

## Host

`apps/host` owns the loopback HTTP/SSE server, Provider registry/runtimes, live state, Phase 3A SQLite boundary, Phase 3B.1 durable Project registry, Phase 3C.1 durable Conversation index/title rules, Phase 3C.1.1 cold detail/hydration boundary, Phase 3D.1 durable Attention projection, Phase 4E.1 Conversation organization model, Phase 4F.1 durable Search projection/read, Phase 5A Provider selection, Phase 6D canonical failure/health composition, and the Controller Relay client. It allocates public identities, maps them to private Provider identities, validates actions and Project workspaces, reconstructs bounded live or cold views from durable Turn records, applies Provider-independent organization writes, searches durable title/User-input text without runtime admission, hydrates control state on demand, selects only the immutable owning adapter for control, validates structured upstream failures, records bounded health observations, sequences client events, and performs bounded fanout/replay. Relay configuration and connection state stay separate from direct endpoint truth and Provider runtime ownership. The Host's Machine coordinator owns direct-first route selection before an operation begins and never changes the selected transport for an active session or Turn. The same Host entry supports standalone Browser development and the production Desktop sidecar; Desktop-managed mode changes lifecycle wiring and Origin input, not business behavior.
...
`packages/protocol` defines Protocol v1 identifiers, commands, records, events, safe errors with strict `CanonicalFailure`, Provider descriptors/capabilities with optional bounded execution health, canonical Tool kinds, ordering, reconnect cursors, strict Project and Project Location records/mutations, the Project-scoped `ConversationSummary`, bounded durable Conversation-detail response, Conversation organization mutations, durable Conversation Search query/result/cursor contracts, durable Attention contracts, and bounded presentation-safe Relay/transport status with TypeScript and Zod. `packages/client` implements the matching HTTP/SSE consumer—including Project Location registration, remote Provider refresh, Conversation organization, Search, Attention, ordinary Conversation creation, and Host-owned Relay configuration/enrollment/status—without React or Provider-specific or Relay-socket methods. Purpose-specific Host ↔ Node Location-validation, Provider-description, Codex execution, and Claude execution envelopes remain private to `packages/machine-transport` and run unchanged over either direct TLS or a Relay-provided Duplex. The separate Internet envelope remains in `packages/relay-protocol`; version 2 adds only the fixed `machine_tls_v1` channel family and cannot name or decode a Machine operation. Raw Provider payloads and private native session identities remain behind Node/Host adapters and are visible to neither Web nor the Relay application. Strict mixed-revision compatibility is not silently negotiated: an incompatible Protocol or Relay peer fails closed instead of weakening validation or mutating durable trust.
...
- Codex remains the accepted full-control integration and was verified against local `codex-cli 0.149.1`. Phase 5A's Claude Code foundation is frozen at `a1329f2`, Phase 5B's evidence-driven capability matrix is accepted and frozen at `5dac13c`, remote reliability is accepted and frozen at Phase 6C.4 commit `01e749c`, Phase 6D diagnostics are accepted and frozen at `cb2c412`, and Phase 7A is accepted and frozen at `67e2a98`. Phase 7B changes transport only and cannot broaden Provider capabilities.
