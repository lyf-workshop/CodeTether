...
**Phase 6C.1 — Remote Provider Discovery & Capability Foundation** is the current approved implementation scope; it is not yet accepted, frozen, or declared ready. Accepted Phase 6B.3 retains one durable Location per Project/Machine and its explicit safe removal semantics. Phase 6C.1 adds one purpose-specific authenticated remote Provider-description message, bounded Node-owned version probes, a presentation-safe last-known snapshot, and Machine UI freshness metadata. Remote Provider execution, generic process invocation, Prompt/session/Turn transport, filesystem access, synchronization, LAN discovery, and relay do not cross the boundary.
...
Phase 6B.1 adds a separate `apps/node` process rather than duplicating the Host. The Node persists random CodeTether-owned Machine and Node identities plus a P-256 TLS identity; none derive from hostname, address, hardware, process, or installation identity. Its framed protocol is a closed union rather than generic RPC. Phase 6B.3 adds one authenticated Project Location validation request to the accepted pairing, identity/status heartbeat, endpoint-recovery, and trust-revocation messages. Phase 6C.1 adds one authenticated Provider-description request whose implementation can run only the two fixed version probes owned by Node code. It has no Project database, Conversation, Provider runtime, shell, generic filesystem, arbitrary process, or Terminal handler. One Node accepts at most one trusted Controller in this phase.
...
Project discovery/import UX, Location relocation, multi-root Locations on one Machine, durable action logging, full-history pagination, and remote Project/Provider execution remain planned rather than implemented. Remote identity/trust, bounded authenticated endpoint recovery, purpose-specific remote Location validation, and bounded Provider description exist only at the Machine connection boundary.

## Client-to-Host Protocol

`packages/protocol` defines Protocol v1 identifiers, commands, records, events, safe errors, Provider descriptors/capabilities, canonical Tool kinds, ordering, reconnect cursors, strict Project and Project Location records/mutations, the Project-scoped `ConversationSummary`, bounded durable Conversation-detail response, Conversation organization mutations, durable Conversation Search query/result/cursor contracts, and durable Attention contracts with TypeScript and Zod. `packages/client` implements the matching HTTP/SSE consumer—including Project Location registration, remote Provider refresh, Conversation organization, Search, Attention, and Provider-aware ordinary Conversation creation—without React or Provider-specific methods. The purpose-specific Host ↔ Node Location-validation and Provider-description envelopes remain private to `packages/machine-transport`; Provider runtime wire payloads remain behind adapters, and unknown Provider notifications are diagnostics rather than public events.
