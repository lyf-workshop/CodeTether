...
**Phase 6B.3 — Remote ProjectLocation** is the current approved implementation scope; it is not yet accepted, frozen, or declared ready. One logical Project may own one durable Location per Machine. The local Host remains the sole Project/Location database owner and registers a remote canonical root only after the trusted online Node completes one purpose-specific authenticated absolute-path, `realpath`, and existing-directory validation request. Remote Provider execution, generic filesystem access, synchronization, discovery, and relay do not cross the boundary.

No Project discovery/scanning, Project or Location rename/relocate/remove, multi-root Location on one Machine, cross-Project/global Search, Agent/Tool/Terminal/Diff/Approval or semantic Search, Search history/analytics, Conversation Delete/bulk organization/tags/folders/groups/old-Turn history pagination, read/unread Inbox history, remote execution/discovery/relay, remote Provider data, Machine switching, notifications after explicit application exit, updater, OpenCode/third Provider, cross-Provider handoff, Claude capability parity, or general filesystem bridge exists. SQLite remains local Host product state, not a remote or multi-user service.
...
Phase 6B.1 adds a separate `apps/node` process rather than duplicating the Host. The Node persists random CodeTether-owned Machine and Node identities plus a P-256 TLS identity; none derive from hostname, address, hardware, process, or installation identity. Its framed protocol is a closed union rather than generic RPC. Phase 6B.3 adds one authenticated Project Location validation request to the accepted pairing, identity/status heartbeat, endpoint-recovery, and trust-revocation messages. It has no Project database, Conversation, Provider, shell, generic filesystem, process, or Terminal handler. One Node accepts at most one trusted Controller in this phase.
...
Phase 6B.3 reuses the existing version-10 `projects` and `project_locations` schema, whose `(project_id, machine_id)` primary key and per-Machine canonical-root uniqueness already support one logical Project on multiple Machines. No schema migration is required. Host SQLite remains authoritative; the Node owns no duplicate Project record. Existing Project reads expose bounded presentation-safe Location summaries, and `POST /api/v1/projects/:projectId/locations` accepts an idempotency action, exact trusted remote `machineId`, and bounded path. The Host requires active trust and online authenticated transport before requesting validation. The authenticated validation operation is serialized per Machine; the subsequent durable insert and the opposing unpair transition each recheck trust/Location constraints atomically in SQLite, so neither can leave a dangling reference.
...
A trusted remote Machine therefore advertises `projectAccess: true` and retains `providerExecution: false`; every other remote execution/native capability remains false. Project/Location and Machine-detail reads do not contact the Node, start a Provider, hydrate a Conversation, or poll the remote directory. A stored Location remains visible while its Machine is offline. Because Phase 6B.3 has no Location-removal operation and Machine deletion would create dangling ownership, unpair fails with `machine_has_project_locations` until a later explicit removal design exists.
...
Project discovery/import UX, Location removal/relocation, multi-root Locations on one Machine, durable action logging, full-history pagination, and remote Project/Provider execution remain planned rather than implemented. Remote identity/trust, bounded authenticated endpoint recovery, and purpose-specific remote Location validation exist only at the Machine connection boundary.

## Client-to-Host Protocol

`packages/protocol` defines Protocol v1 identifiers, commands, records, events, safe errors, Provider descriptors/capabilities, canonical Tool kinds, ordering, reconnect cursors, strict Project and Project Location records/mutations, the Project-scoped `ConversationSummary`, bounded durable Conversation-detail response, Conversation organization mutations, durable Conversation Search query/result/cursor contracts, and durable Attention contracts with TypeScript and Zod. `packages/client` implements the matching HTTP/SSE consumer—including Project Location registration, Conversation organization, Search, Attention, and Provider-aware ordinary Conversation creation—without React or Provider-specific methods. The purpose-specific Host ↔ Node Location-validation envelope remains private to `packages/machine-transport`; Provider wire payloads remain behind adapters, and unknown Provider notifications are diagnostics rather than public events.
