...
**Phase 6C.2 — Remote Codex Execution Foundation** is the current approved implementation scope; it is not yet accepted, frozen, or declared ready. Phase 6C.1 is accepted and frozen at `466d63c`; its purpose-specific authenticated Provider description, bounded Node-owned probes, durable last-known snapshot, and freshness semantics remain unchanged. Phase 6C.2 adds only a fixed remote Codex text-streaming/native-resume session over the pinned Machine transport for an exact durable Conversation and registered Project Location. Remote Claude Code, Tools, edit/write, shell, Diff, Approval, interrupt, model/reasoning control, arbitrary process/argv/environment/cwd, generic filesystem access/RPC, synchronization, LAN discovery, and relay do not cross the boundary.

No Project discovery/scanning, Project or Location rename/relocate, multi-root Location on one Machine, cross-Project/global Search, Agent/Tool/Terminal/Diff/Approval or semantic Search, Search history/analytics, Conversation Delete/bulk organization/tags/folders/groups/old-Turn history pagination, read/unread Inbox history, remote execution/discovery/relay, remote Provider data, Machine switching, notifications after explicit application exit, updater, OpenCode/third Provider, cross-Provider handoff, Claude capability parity, or general filesystem bridge exists. SQLite remains local Host product state, not a remote or multi-user service.
...
Local Host on 127.0.0.1
       ...
       └── Secure Machine Connection Boundary
                 ...
                 ▼
          CodeTether Node
          └── remote identity/status only in Phase 6B.1
```
...
Phase 6B.1 adds a separate `apps/node` process rather than duplicating the Host. The Node persists random CodeTether-owned Machine and Node identities plus a P-256 TLS identity; none derive from hostname, address, hardware, process, or installation identity. Its framed protocol is a closed union rather than generic RPC. Phase 6B.3 adds one authenticated Project Location validation request to the accepted pairing, identity/status heartbeat, endpoint-recovery, and trust-revocation messages. Accepted Phase 6C.1 adds one authenticated Provider-description request whose implementation can run only the two fixed version probes owned by Node code. Phase 6C.2 adds a dedicated bounded authenticated Codex session connection: the Controller supplies only exact product identities, the registered canonical root, an optional private native thread identity, one action identity, and bounded Prompt text. The Node still has no Project/Conversation/Search/Attention database, shell, generic filesystem, arbitrary process/argv/environment/cwd, or Terminal handler. One Node accepts at most one trusted Controller.
...
Phase 6C.2 extends that same closed union only with exact Codex session open/ready/dispose and Turn start/started/event messages. The session binds protocol, pinned Machine/Node, public Conversation/Project, registered root, and private native thread identity; the Turn binds one action, Conversation, public Turn, private Provider identities, and bounded Prompt. No caller selects executable, argv, environment, model, shell, arbitrary cwd, or method name. The Node revalidates the stored canonical directory immediately before each Prompt, launches the fixed tested text-only profile with `shell: false`, and rejects every unexpected Tool/file/Approval/server request/raw notification before it can cross the transport. Output framing, queue bytes, total output, concurrent sessions, and lifetime are bounded. Ambiguous loss is terminal and never causes automatic Prompt replay.
...
Local Provider execution ownership remains in `ProviderRegistry`; remote discovery belongs to `RemoteMachineCoordinator` and never creates a runtime adapter, session, Conversation, or hydration slot. CLI availability alone therefore leaves `providerExecution: false`. Phase 6C.2 sets it true only for an online authenticated Machine whose current Codex descriptor exposes the separately tested streaming/resume execution profile.
...
Project discovery/import UX, Location relocation, multi-root Locations on one Machine, durable action logging, full-history pagination, remote Claude execution, and richer remote Codex capability parity remain planned rather than implemented. Remote identity/trust, bounded authenticated endpoint recovery, purpose-specific remote Location validation, Provider description, and the fixed Codex text execution profile exist only at the Machine connection boundary.

## Client-to-Host Protocol

`packages/protocol` defines Protocol v1 identifiers, commands, records, events, safe errors, Provider descriptors/capabilities, canonical Tool kinds, ordering, reconnect cursors, strict Project and Project Location records/mutations, the Project-scoped `ConversationSummary`, bounded durable Conversation-detail response, Conversation organization mutations, durable Conversation Search query/result/cursor contracts, and durable Attention contracts with TypeScript and Zod. `packages/client` implements the matching HTTP/SSE consumer—including Project Location registration, remote Provider refresh, Conversation organization, Search, Attention, and Provider/Machine-aware ordinary Conversation creation—without React or Provider-specific public methods. The purpose-specific Host ↔ Node Location-validation, Provider-description, and Codex execution envelopes remain private to `packages/machine-transport`; raw Provider payloads and private native session identities remain behind Node/Host adapters and never become Web data.
...
Phase 6C.2 needs no migration: the existing immutable Machine/Project/Provider binding, private `provider_thread_id`, Turn snapshot, Search, organization, and Attention graph already own durable remote execution truth. The private native Codex identity remains presentation-inaccessible.
...
- The Host API remains a loopback-only service with local SQLite records. Browser development may launch it separately; CodeTether Desktop packages and owns the same Host as a revision-coupled sidecar. Only the separate bounded Machine connection client reaches a manually specified CodeTether Node on the LAN.
- Codex remains the accepted full-control integration and was verified against local `codex-cli 0.149.1`. Phase 5A's Claude Code foundation is frozen at `a1329f2`, and Phase 5B's evidence-driven capability matrix is accepted and frozen at `5dac13c`.
