...
**Phase 8A — Existing Session Discovery & Adoption** is the current approved implementation scope. It adds request-scoped, Machine-local, Provider-specific metadata discovery for exact Project Locations; short-lived opaque public candidates; and atomic, idempotent adoption into the existing Conversation/private native-session binding. Discovery and adoption are read-only and perform no inference. Only a later explicit Turn uses the frozen native-resume path. Phase 8A adds no transcript replay, generic file access, Provider lifecycle manager, backend/profile manager, onboarding redesign, or Mobile behavior.
...
`packages/protocol` defines Protocol v1 identifiers, commands, records, events, safe errors with strict `CanonicalFailure`, Provider descriptors/capabilities with optional bounded execution health, canonical Tool kinds, ordering, reconnect cursors, strict Project and Project Location records/mutations, the Project-scoped `ConversationSummary`, bounded durable Conversation-detail response, Conversation organization mutations, durable Conversation Search query/result/cursor contracts, durable Attention contracts, bounded presentation-safe Relay/transport status, and Phase 8A's opaque Provider-session discovery/adoption contracts with TypeScript and Zod. `packages/client` implements the matching HTTP/SSE consumer—including Project Location registration, remote Provider refresh, opaque previous-session discovery/adoption, Conversation organization, Search, Attention, ordinary Conversation creation, and Host-owned Relay configuration/enrollment/status—without React, native-session identity, or Relay-socket methods. Purpose-specific Host ↔ Node Location validation, Provider description, native-session discovery/revalidation, Codex execution, and Claude execution envelopes remain private to `packages/machine-transport` and run over either direct TLS or a Relay-provided Duplex. The separate Internet envelope remains in `packages/relay-protocol`; version 2 adds only the fixed `machine_tls_v1` channel family and cannot name or decode a Machine operation. Raw Provider payloads and private native session identities remain behind Node/Host adapters and are visible to neither Web nor the Relay application. Strict mixed-revision compatibility is not silently negotiated: an incompatible Protocol or Relay peer fails closed instead of weakening validation or mutating durable trust.
...
`packages/adapter-claude` safely resolves a native executable or verified npm installation, then performs bounded `--version` and JSON authentication-status detection once per Host lifecycle. Phase 5A was accepted against Claude Code `2.1.250`; Phase 5B revalidated the same boundary against `2.1.251`; Phase 8A revalidated the unchanged capability boundary against `2.1.263`. Those exact verified versions remain admitted, while any other detected version is reported truthfully instead of being assumed fully supported. A Node lifecycle selects the first successfully resolved canonical Claude launcher once, and version observation plus every later execution use that same private launcher identity until Node restart; failed resolution remains eligible for the existing bounded refresh, but a selected installation never silently falls through to another installation. Session discovery uses the same lifecycle configuration environment while remaining read-only and executable-independent. Detection is a Host-lifecycle capability snapshot, while a later launch/authentication failure remains a canonical Provider mutation error. The adapter never publishes the path or raw diagnostics. Available Claude sessions use a UUID with native `--session-id` creation and `--resume` after Host restart. One active Turn owns one child; canonical input is encoded as a JSONL SDK User message on stdin, not placed in argv, and the process uses structured arguments with `shell: false` in the already authorized Conversation working directory.
