...
**Phase 6B.3 — Remote ProjectLocation** is the current approved implementation scope; it is not yet accepted, frozen, or declared ready. One logical Project may own one durable Location per Machine. The local Host remains the sole Project/Location database owner and registers a remote canonical root only after the trusted online Node completes one purpose-specific authenticated absolute-path, `realpath`, and existing-directory validation request. The Host can explicitly remove an unreferenced remote Location as Controller-owned metadata without contacting the Node; the canonical local Location and Project/Machine pairs referenced by durable Conversations fail closed. Remote Provider execution, generic filesystem access, synchronization, discovery, and relay do not cross the boundary.
