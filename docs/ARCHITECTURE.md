...
**Phase 6D — Provider Failure Diagnostics & Recovery UX** is the current approved implementation scope; it is not yet accepted, frozen, or declared ready. It layers one strict CodeTether-owned `CanonicalFailure` model, durable failed-Turn metadata, bounded recent Provider execution health, upstream Codex/Claude/transport/runtime classification, and controlled recovery presentation over the frozen local and remote execution paths. It adds no Provider capability, credential flow, automatic login, Provider/Machine/directory fallback, automatic Prompt replay, continuous Provider polling, raw-log surface, Relay, Internet transport, Mobile behavior, or later-phase remote control.
...
`packages/protocol` defines Protocol v1 identifiers, commands, records, events, safe errors with strict `CanonicalFailure`, Provider descriptors/capabilities with optional bounded execution health, canonical Tool kinds, ordering, reconnect cursors, strict Project and Project Location records/mutations, the Project-scoped `ConversationSummary`, bounded durable Conversation-detail response, Conversation organization mutations, durable Conversation Search query/result/cursor contracts, and durable Attention contracts with TypeScript and Zod. `packages/client` implements the matching HTTP/SSE consumer—including Project Location registration, remote Provider refresh, Conversation organization, Search, Attention, and Provider/Machine-aware ordinary Conversation creation—without React or Provider-specific public methods. The purpose-specific Host ↔ Node Location-validation, Provider-description, Codex execution, and Claude execution envelopes remain private to `packages/machine-transport`; raw Provider payloads and private native session identities remain behind Node/Host adapters and never become Web data. Phase 6D deployment validation uses revision-matched packaged Web, Host, and Node builds. Strict mixed-revision rolling compatibility is not negotiated inside Protocol v1; an incompatible peer fails closed instead of weakening validation or mutating durable trust.
...
- Codex remains the accepted full-control integration and was verified against local `codex-cli 0.149.1`. Phase 5A's Claude Code foundation is frozen at `a1329f2`, Phase 5B's evidence-driven capability matrix is accepted and frozen at `5dac13c`, and remote reliability is accepted and frozen at Phase 6C.4 commit `01e749c`. Phase 6D diagnostics may explain these capabilities and failures but cannot broaden them.
