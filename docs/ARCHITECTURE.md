...
**Phase 4F.1 — Durable Conversation Search Model & API** is implemented; Owner acceptance is pending. SQLite migration 006 owns a normalized title/canonical-User-input projection, while additive Protocol v1 and Client contracts expose a strict Project-scoped, filtered, cursor-paginated Search read. Search never depends on Runtime hydration, provider availability, Project filesystem availability, or `snapshot_json`, and no formal React Search UI is connected in this phase.

No Project discovery/scanning or rename/relocate, formal durable/global Search UI, Agent/Tool/Terminal/Diff/Approval or semantic Search, Conversation Delete/bulk organization/tags/folders/groups/history pagination, read/unread Inbox history, remote access, authentication, multiple-Machine Project model, tray, closed-app notification delivery, updater, additional provider, or general filesystem bridge exists. SQLite remains local Host data, not a remote or multi-user service.
