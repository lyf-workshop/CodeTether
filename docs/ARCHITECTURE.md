...
Doctor's explicit remote ProjectLocation check uses the existing narrow folder-validation operation on a bounded, pinned connection without replacing the heartbeat or refreshing Provider metadata. This avoids demoting a current backend execution observation merely by checking the folder. Execution admission and explicit Provider refresh retain their accepted lifecycle behavior; Doctor never promotes last-known health or opens a Provider session.
