...
**Phase 6A — Durable Machine Foundation** is the current approved implementation scope; it is not yet accepted, frozen, or declared ready. SQLite migration 008 creates exactly one durable local Machine, separates logical Project identity from a Machine-scoped authorized Project Location, and backfills every Conversation with the same required immutable Machine identity. Protocol v1/Client add strict Machine list/detail reads and require `machineId` for ordinary Conversation creation. The Host keeps Machine and Provider registries separate, composes Provider availability for the selected Machine, and exposes real Machine/Location truth to shared Web/Desktop UI without Provider hydration, a remote transport, monitoring loop, or native capability change.

No Project discovery/scanning or rename/relocate, second Project Location, cross-Project/global Search, Agent/Tool/Terminal/Diff/Approval or semantic Search, Search history/analytics, Conversation Delete/bulk organization/tags/folders/groups/old-Turn history pagination, read/unread Inbox history, remote transport/trust/pairing/discovery, authentication, adding or switching Machines, notifications after explicit application exit, updater, OpenCode/third Provider, cross-Provider handoff, Claude capability parity, or general filesystem bridge exists. SQLite remains local Host data, not a remote or multi-user service.
