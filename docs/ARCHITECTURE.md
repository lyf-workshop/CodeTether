...
**Phase 6B.2 — Secure Connection Recovery & Address Mobility** is the current approved implementation scope; it is not yet accepted, frozen, or declared ready. Trust continues to be the pinned Node identity rather than an address. The Host may retain a bounded private set of authenticated endpoint hints, try them through one cancellable per-Machine reconnect coordinator, and authenticate a manually supplied recovery address before promoting it. No Project, Conversation, Provider, Turn, shell, filesystem, generic RPC, discovery service, or relay crosses the remote boundary.

No Project discovery/scanning or rename/relocate, second Project Location, cross-Project/global Search, Agent/Tool/Terminal/Diff/Approval or semantic Search, Search history/analytics, Conversation Delete/bulk organization/tags/folders/groups/old-Turn history pagination, read/unread Inbox history, remote execution/discovery/relay, remote Project/Provider data, Machine switching, notifications after explicit application exit, updater, OpenCode/third Provider, cross-Provider handoff, Claude capability parity, or general filesystem bridge exists. SQLite remains local Host product state, not a remote or multi-user service.
...
Phase 6B.1 adds a separate `apps/node` process rather than duplicating the Host. The Node persists random CodeTether-owned Machine and Node identities plus a P-256 TLS identity; none derive from hostname, address, hardware, process, or installation identity. Its only framed messages are pairing, authenticated identity/status heartbeat, and trust revocation. It has no Project, Conversation, Provider, shell, filesystem, process, Terminal, or generic RPC handler. One Node accepts at most one trusted Controller in this phase.
...
Project discovery/import UX, multiple Machine locations, durable action logging, full-history pagination, and remote Project/Provider execution remain planned rather than implemented. Remote identity/trust and bounded authenticated endpoint recovery exist only at the Machine connection boundary.
