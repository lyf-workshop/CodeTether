...
**Phase 6B.1 — Remote Node Identity & Secure Pairing** is the current approved implementation scope; it is not yet accepted, frozen, or declared ready. A minimal CodeTether Node owns a durable random Machine identity and private cryptographic identity. The local Host coordinates explicit short-lived PAKE pairing, user confirmation, durable pinned trust, mutually authenticated reconnect, bounded heartbeat, truthful availability, and unpair. The remote wire protocol is private to the Host/Node boundary; Web still uses typed Protocol v1. No Project, Conversation, Provider, Turn, shell, filesystem, or generic RPC crosses the remote boundary.
...
The Host resolves a manually supplied endpoint once, requires every resolved address to be private/link-local LAN space, and never modifies firewall policy or performs discovery. A single bounded worker per trusted Machine maintains authenticated liveness with exponential backoff and publishes low-frequency `machine.updated` transitions. Remote capabilities remain false, so Machine reads, pairing, reconnect, and unpair cannot create a Project Location, select a Provider, consume a Runtime slot, or start Codex/Claude.
...
The Tauri capability boundary exposes only the exact directory-picker and bounded Attention-notification commands to React and no generic native command surface. Production CSP is explicit and loopback-only, while Rust owns the fixed Host sidecar command and private lifecycle pipe. A selected path still crosses the existing Project API before it becomes an authorized workspace; a notification intent can only display safe copy and later return public routing identities. The local business API remains loopback-only. Phase 6B.1's separate outbound Host ↔ Node boundary uses an encrypted, authenticated, strictly framed protocol with only pairing, identity/status heartbeat, and revocation messages; it does not expose the loopback HTTP API or any generic RPC to the LAN.
