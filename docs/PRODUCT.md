...
**Phase 2C.1 — Live Conversation Read Model** connects only the frozen Desktop Conversation Detail read path to that Host boundary. A real `conv_*` route loads bootstrap and an in-memory snapshot, follows normalized Host events over one application-scoped SSE runtime, projects them into the same view model used by the Demo, and renders the result read-only. `/conversations/demo`, Inbox, and Conversations continue to use Mock data. Composer submission, approval resolution, interruption, and every other React write path remain deferred to Phase 2C.2.

This read model is live rather than durable. The current Snapshot contains Conversation summaries, active Turns, and pending Approvals, but not prior message, Tool, terminal, file-change, or diff Item history and not the user's Turn input. Refresh, Host restart, or `stream.reset` therefore rebuilds only the state represented by that Snapshot; it cannot reconstruct already-evicted live timeline content. Phase 2C.1 adds no persistence, Tauri functionality, remote access, authentication, or non-Codex provider integration. See [`ROADMAP.md`](ROADMAP.md) for phase gates.
